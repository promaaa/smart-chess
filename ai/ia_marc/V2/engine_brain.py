...
import chess
from bitboard_utils import get_queen_attacks
...
MG_QUEEN = [-28,0,29,12,59,44,43,45,-24,-39,-5,1,-16,57,28,54,-13,-17,7,8,29,56,47,57,-27,-27,-16,-16,-1,17,-2,1,-9,-26,-9,-10,-2,-4,3,-3,-14,2,-11,-2,-5,2,14,5,-35,-8,11,2,8,15,-3,1,-1,-18,-9,-10,-30,-15,-13,-32]
...
EG_BISHOP = [-14,-21,-11,-8,-7,-9,-17,-24,-8,-4,7,-12,-3,-13,-4,-14,-4,-8,-2,-6,-2,-14,-9,-1,-2,-2,-1,-5,-9,10,-5,-6,-9,2,-5,3,8,-3,2,-10,-13,-5,-2,3,-8,-12,-7,-8,-9,-14,1,0,-6,-3,-18,-14,-23,-9,-23,-5,-9,-16,-5,-17]
EG_ROOK = [13,10,18,15,12,12,8,5,11,13,13,11,12,12,2,7,0,9,9,1,1,5,10,0,-7,-10,-1,6,7,1,2,-5,-6,-13,-7,4,5,5,-4,-14,-13,-16,-13,-8,-5,-14,-14,-10,-19,-8,-7,-11,-9,-12,-15,-14,-9,-8,-9,-14,-14,-5,-14,-13]
...
raw_mg = {chess.PAWN: MG_PAWN, chess.KNIGHT: MG_KNIGHT, chess.BISHOP: MG_BISHOP, chess.ROOK: MG_ROOK, chess.QUEEN: MG_QUEEN, chess.KING: MG_KING}
raw_eg = {chess.PAWN: EG_PAWN, chess.KNIGHT: EG_KNIGHT, chess.BISHOP: EG_BISHOP, chess.ROOK: EG_ROOK, chess.QUEEN: EG_QUEEN, chess.KING: EG_KING}
...
class EvaluationEngine:
    ...
    def evaluate(self, board: chess.Board) -> int:
        ...
        if self.use_mobility:
            mg_mobility, eg_mobility = self._evaluate_mobility(board)
        ...
        if self.use_pawn_structure:
            ...
            mg_score += pawn_score
            eg_score += pawn_score
        if self.use_king_safety and phase > 12:
    ...
    def _evaluate_mobility(self, board: chess.Board) -> tuple:
        mg_score = 0
        eg_score = 0
        mg_weights = {chess.KNIGHT: 4, chess.BISHOP: 5, chess.ROOK: 2, chess.QUEEN: 1}
        eg_weights = {chess.KNIGHT: 4, chess.BISHOP: 5, chess.ROOK: 3, chess.QUEEN: 2}
        for color in [chess.WHITE, chess.BLACK]:
            for piece_type in [chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN]:
                for square in board.pieces(piece_type, color):
                    mobility = len(board.attacks(square))
                    if color == chess.WHITE:
                        mg_score += mobility * mg_weights[piece_type]
                        eg_score += mobility * eg_weights[piece_type]
                    else:
                        mg_score -= mobility * mg_weights[piece_type]
                        eg_score -= mobility * eg_weights[piece_type]
        return mg_score // 10, eg_score // 10

    def _evaluate_pawn_structure(self, board: chess.Board) -> int:
        score = 0
        white_pawns = board.pieces(chess.PAWN, chess.WHITE)
        black_pawns = board.pieces(chess.PAWN, chess.BLACK)
        score += self._evaluate_pawns_for_color(board, white_pawns, black_pawns, chess.WHITE)
        score -= self._evaluate_pawns_for_color(board, black_pawns, white_pawns, chess.BLACK)
        return score

    def _evaluate_pawns_for_color(self, board: chess.Board, my_pawns: chess.SquareSet, enemy_pawns: chess.SquareSet, color: chess.Color) -> int:
        score = 0
        king_sq = board.king(color)
        king_file = chess.square_file(king_sq) if king_sq is not None else 4
        my_pawn_files = {chess.square_file(sq) for sq in my_pawns}
        for square in my_pawns:
            file = chess.square_file(square)
            rank = chess.square_rank(square)
            direction = 1 if color == chess.WHITE else -1
            if len([s for s in my_pawns if chess.square_file(s) == file]) > 1:
                score -= 10
            if (file - 1) not in my_pawn_files and (file + 1) not in my_pawn_files:
                score -= 10
            front_square = chess.square(file, rank + direction)
            if (1 << front_square) & board.occupied:
                score -= 5
            is_passed = True
            ...
            if king_sq is not None:
                is_kingside_pawn = (king_file >= 4 and file >= 4) or (king_file < 4 and file < 4)
                advancement_rank = rank - 1 if color == chess.WHITE else 6 - rank
                if advancement_rank > 2:
                    if is_kingside_pawn:
                        score -= advancement_rank * 4
                    else:
                        score += advancement_rank * 4
        return score
    ...
    def _king_safety_for_square(self, board: chess.Board, king_sq: int, color: chess.Color) -> int:
        score = 0
        enemy_color = not color
        if color == chess.WHITE:
            if king_sq in [chess.G1, chess.C1]: score += 30
        else:
            if king_sq in [chess.G8, chess.C8]: score += 30
        ...
        if file in [3, 4]: score -= 20
        
        # Penalty for king exposure based on queen-like attacks
        occupied = board.occupied
        queen_attacks = get_queen_attacks(king_sq, occupied)
        score -= bin(queen_attacks).count("1")
        
        king_zone = chess.BB_KING_ATTACKS[king_sq]
        attack_penalty = 0
        attack_weights = {chess.KNIGHT: 3, chess.BISHOP: 3, chess.ROOK: 5, chess.QUEEN: 8}
        for piece_type, weight in attack_weights.items():
            for attacker_square in board.pieces(piece_type, enemy_color):
                attacks_in_zone = bin(board.attacks(attacker_square) & king_zone).count("1")
                if attacks_in_zone > 0:
                    attack_penalty += attacks_in_zone * weight
