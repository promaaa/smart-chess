...
def find_mate_in_one_or_two(board: chess.Board):
    ...
    for move in board.legal_moves:
        ...
        if board.is_checkmate():
            board.pop()
            return move
        # Mat en 2 : simule tous les coups adverses
...
def king_safety_score(board: chess.Board, color: chess.Color) -> int:
    ...
    front_pawns = [chess.square(file + df, rank + direction)
                   for df in [-1, 0, 1] if 0 <= file + df < 8 and 0 <= rank + direction < 8]
...
def piece_mobility(board: chess.Board, color: chess.Color) -> int:
    """Évalue la mobilité générale des pièces."""
    mobility = 0
...
def evaluate(board: chess.Board) -> int:
    """Évaluation hybride complète avec sécurité du roi et activité."""
    piece_values = {
    ...
    # --- 4️⃣ Développement / ouverture ---
    if board.fullmove_number <= 12:
        for color in [chess.WHITE, chess.BLACK]:
            # Dame sortie trop tôt
    ...
    # --- 7️⃣ Sécurité du roi & mobilité ---
    score += king_safety_score(board, chess.WHITE)
    score -= king_safety_score(board, chess.BLACK)
    score += 4 * piece_mobility(board, chess.WHITE)
    score -= 4 * piece_mobility(board, chess.BLACK)

    # --- 8️⃣ Bonus progression finale ---
    if endgame:
        for color in [chess.WHITE, chess.BLACK]:
            for sq in board.pieces(chess.PAWN, color):
                rank = chess.square_rank(sq)
                defenders = sum(1 for s in board.piece_map()
                                if board.color_at(s) == color and sq in board.attacks(s))
                if defenders > 0 and rank >= 5:
                    score += (30 + 10 * defenders) if color == chess.WHITE else -(30 + 10 * defenders)

    # --- 9️⃣ No progress / draw ---
    if board.halfmove_clock > 50:
        score -= (board.halfmove_clock - 50) * 2
...
def find_best_move(board, max_plies=MAX_PLIES, time_limit=TIME_LIMIT, tt=None):
    ...
    # ⚡️ Vérification tactique mat en 1 ou 2 coups
    ...
    # Iterative deepening — on travaille SUR UNE COPIE pour éviter d'altérer le board principal
    for depth in range(1, adaptive_max + 1):
        ...
        try:
            # Utiliser une copie du board pour la recherche (zéro risque de désynchronisation)
            search_board = board.copy()
            val, move = negamax(search_board, depth, -10**7, 10**7, tt, start_time, adaptive_time)
            if move:
                best_move = move
                best_score = val
                best_depth = depth
