...
import random
import sys
from eval_fast import eval_core   # module compilé par Pythran
...
def evaluate(board: chess.Board) -> int:
    ...
    material_white = sum(value[p.piece_type] for p in board.piece_map().values() if p.color)
    material_black = sum(value[p.piece_type] for p in board.piece_map().values() if not p.color)
    ...
    for sq, p in board.piece_map().items():
        if p.piece_type == chess.PAWN:
            ...
            table = KING_MID if len(board.piece_map()) > 12 else KING_END

        if p.color:
            pst_white += table[sq]
        else:
            pst_black += table[63 - sq]   # miroir

    # Pions doublés
    doubled_white = 0
    doubled_black = 0
    for file in range(8):
        white_on_file = [sq for sq in board.pieces(chess.PAWN, chess.WHITE) if chess.square_file(sq)==file]
        black_on_file = [sq for sq in board.pieces(chess.PAWN, chess.BLACK) if chess.square_file(sq)==file]
        if len(white_on_file) > 1: doubled_white += 1
        if len(black_on_file) > 1: doubled_black += 1
        ...
        for df in [-1,0,1]:
            ...
            if 0<=nf<8:
                for nr in range(r+1,8):
                    if chess.square(nf,nr) in board.pieces(chess.PAWN, chess.BLACK):
                        passed = False
                        break
        if passed: passed_white += 1
        ...
        for df in [-1,0,1]:
            ...
            if 0<=nf<8:
                for nr in range(r-1,-1,-1):
                    if chess.square(nf,nr) in board.pieces(chess.PAWN, chess.WHITE):
                        passed = False
                        break
        if passed: passed_black += 1

    # Tours en colonnes ouvertes
    rook_open_white = sum(1 for sq in board.pieces(chess.ROOK, chess.WHITE)
                          if all(chess.square_file(p)!=chess.square_file(sq)
                                 for p in board.pieces(chess.PAWN, chess.WHITE)))

    rook_open_black = sum(1 for sq in board.pieces(chess.ROOK, chess.BLACK)
                          if all(chess.square_file(p)!=chess.square_file(sq)
                                 for p in board.pieces(chess.PAWN, chess.BLACK)))

    # Sécurité du roi
    king_safety_white = -10 if board.is_attacked_by(chess.BLACK, board.king(chess.WHITE)) else 0
    king_safety_black = -10 if board.is_attacked_by(chess.WHITE, board.king(chess.BLACK)) else 0

    # Phase (0 = finale, 100 = ouverture)
    phase = min(100, len(board.piece_map()))
...
def find_best_move(board, max_plies=MAX_PLIES, time_limit=TIME_LIMIT, tt=None):
    ...
    if tactical_move:
        if DEBUG_PRINT:
            print(f"[TACTIC] Mat détecté avec {tactical_move.uci()}", file=sys.stderr)
    ...
    for depth in range(1, adaptive_max + 1):
        ...
        try:
            ...
            val, move = negamax(search_board, depth, -10**7, 10**7,
